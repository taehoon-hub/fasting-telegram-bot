  ...
  try {
    ...
    if (existing) {
      await bot.sendMessage(msg.chat.id, `진행 중인 공복이 있습니다.\n\n목표시간: ${existing.targetHours}시간\n\n기존 공복을 이어서 진행하시겠습니까?`, {
        reply_markup: keyboard([[button('이어서 진행', `resume:${existing.id}`)], [button('중지하고 새로 시작', `restart:${existing.id}`)]])
      });
      return;
    }
...
bot.on('message', async (msg) => {
  ...
  await bot.sendMessage(msg.chat.id, `이름: ${state.name}\n그룹: ${state.groupTag}\n\n공복 시간을 선택해 주세요.`, {
    reply_markup: keyboard([[button('12시간', 'target:12'), button('14시간', 'target:14')], [button('16시간', 'target:16'), button('18시간', 'target:18')], [button('20시간', 'target:20')]])
  });
});
  ...
  try {
    ...
    if (data.startsWith('target:')) {
      ...
      await bot.editMessageText(`이름: ${state.name}\n그룹: ${state.groupTag}\n목표시간: ${state.targetHours}시간\n\n공복을 시작할까요?`, {
        chat_id: chatId,
        message_id: messageId,
        reply_markup: keyboard([[button('공복 시작', 'confirm_start')], [button('이름 수정', 'start_fasting')]])
      });
      return;
    }
...
app.listen(PORT, () => console.log(`HTTP 서버 시작: ${PORT}`));
